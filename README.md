...
A auth with express, passport, and JWT - a solution for Nodejs applications

Inspired by [Implementing a JWT auth system with TypeScript and Node](https://nozzlegear.com/blog/implementing-a-jwt-auth-system-with-typescript-and-node)
...
#### 1. Create a package project
    ...
    ```bash
    git clone https://github.com/JuliusAgency/auth-jwt.git .
    ```
    Install:
    ```bash
    npm install
...
##### 4.2 Automated publishing:
    ...
    Note: Before running the GitHub Actions, set this two environment variables:  
        GITHUB_TOKEN – go to GitHub and select your repository. Then go to Settings/Actions/General.  
